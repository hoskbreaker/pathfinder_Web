...
echo "funciona?";
$stmt = $db->prepare("SELECT IDpj FROM $tbl_name where nombre = ?");
$id='';
$name=$_POST['nombre'];
...
echo $stmt->bind_result($id);
if(!$stmt->bind_result($id)){//si no existe el ID
	echo "el valor ID es ".$id;
	//inserta el dato
	$insertar = $db->prepare("INSERT INTO $tbl_name (nombre, alineamiento, jugador, clase, nivel, dios, tierra, raza, tamano, sexo, edad, peso, ojos) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
	...
	echo "el valor ID es ".$id;
	//actualiza el dato
	$actualizar = $db->prepare("UPDATE $tbl_name SET nombre=?, alineamiento=?, jugador=?, clase=?, nivel=?, dios=?, tierra=?, raza=?, tamano=?, sexo=?, edad=?, peso=?, ojos=?");
